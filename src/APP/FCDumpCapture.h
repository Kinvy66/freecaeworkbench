 ...
 * @code{.cpp}
 * // 基本使用方法：
 * FC::FCDumpCapture::initDump();
 *
 * // 自定义dump文件路径：
 * FC::FCDumpCapture::initDump([]() -> QString {
 ...
 * FC::FCDumpCapture::initDump(
 *     []() -> QString {
 *         return FC::FCDumpCapture::getDefaultDumpDirectory() + "/myapp_crash.dmp";
 *     },
 ...
 *
 * // 清理7天前的dump文件：
 * FC::FCDumpCapture::cleanupOldDumps(7);
 *
 * // 清理指定目录的dump文件：
 * FC::FCDumpCapture::cleanupOldDumps(7, "C:/MyApp/logs");
 * @endcode
 */
class FCDumpCapture
{
...
public:
     ...
     *
     * @code{.cpp}
     * QString defaultDir = FC::FCDumpCapture::getDefaultDumpDirectory();
     ...
     *
     * // 清理指定目录中30天前的文件
     * FC::FCDumpCapture::cleanupOldDumps(30, "C:/MyApp/logs");
     *
     * // 清理自定义目录中15天前的文件
     * FC::FCDumpCapture::cleanupOldDumps(15, QApplication::applicationDirPath() + "/crash_logs");
     * @endcode
     */
...
#ifdef Q_OS_WIN
#ifdef Q_CC_MSVC
FpPreposeDump FCDumpCapture::s_fp_prepose_dump = nullptr;
FpPostDump FCDumpCapture::s_fp_post_dump       = nullptr;
bool FCDumpCapture::s_initialized              = false;
...
LONG applicationCrashHandler(_EXCEPTION_POINTERS* pException)
{
    ...
    try {
        // 访问FCDumpCapture的静态成员变量
        FpPreposeDump fpDump = FCDumpCapture::getPreposeDumpFunc();
        if (fpDump) {
            createPath = fpDump();
        } else {
            // 默认路径生成
        ...
        FpPostDump fpPostDump = FCDumpCapture::getPostDumpFunc();
        if (fpPostDump) {
