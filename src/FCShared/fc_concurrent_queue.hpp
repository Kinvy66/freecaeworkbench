#ifndef FC_CONCURRENT_QUEUE_H
#define FC_CONCURRENT_QUEUE_H
#include <queue>
#include <mutex>
...
public:
    fc_concurrent_queue();
    fc_concurrent_queue(size_type capacity);
    bool        empty() const;
 ...
 */
template< typename T >
fc_concurrent_queue< T >::fc_concurrent_queue() : m_capacity(0)
{
}
 ...
 */
template< typename T >
fc_concurrent_queue< T >::fc_concurrent_queue(size_type capacity) : m_capacity(capacity)
{
}
 ...
 */
template< typename T >
bool fc_concurrent_queue< T >::empty() const
{
 ...
 */
template< typename T >
void fc_concurrent_queue< T >::push(const T& v)
{
...
template< typename T >
void fc_concurrent_queue< T >::set(const T& v)
{
    push(v);
}
 ...
 */
template< typename T >
T fc_concurrent_queue< T >::get(int waitms)
{
    std::unique_lock< std::mutex > lg(m_mutex);
 ...
 */
template< typename T >
std::size_t fc_concurrent_queue< T >::size() const
{
    std::lock_guard< std::mutex > lg(m_mutex);
