 ...
 */
class FCMessageHandlerGlobalValues_Private
{
    FCMessageHandlerGlobalValues_Private();
...
public:
    ~FCMessageHandlerGlobalValues_Private();
    // 获取单例
    static FCMessageHandlerGlobalValues_Private& getInstance();
    // spdlog日志
...
{
    static FCMessageHandlerGlobalValues_Private s_msg_handle_values;
    return s_msg_handle_values;
}

void FCMessageHandlerGlobalValues_Private::setLogger(const std::shared_ptr< spdlog::logger >& logger)
{
    this->_daLogger = logger;
}

spdlog::logger* FCMessageHandlerGlobalValues_Private::logger()
{
    return this->_daLogger.get();
}

void FCMessageHandlerGlobalValues_Private::setEnableMessageCaptureToQueue(bool on)
{
    return this->_enableCaptureToQueue.store(on);
}

bool FCMessageHandlerGlobalValues_Private::isEnableMessageCaptureToQueue() const
{
    return this->_enableCaptureToQueue.load();
}

void FCMessageHandlerGlobalValues_Private::setEnableSpdLog(bool on)
{
    _enableSpdlog.store(on);
}

bool FCMessageHandlerGlobalValues_Private::enableSpdLog() const
{
...
FCMessageHandlerGlobalValues_Private::MsgHandleType FCMessageHandlerGlobalValues_Private::getMsgHandleType() const
{
    return _type;
}

QtMsgType FCMessageHandlerGlobalValues_Private::getMsgQueueRecordMsgType() const
{
    return _recordMsgType;
}

void FCMessageHandlerGlobalValues_Private::setMsgQueueRecordMsgType(QtMsgType t)
{
    _recordMsgType = t;
}
 ...
 */
void FCMessageHandlerGlobalValues_Private::setPattern(const QString& p)
{
    // 对占位符进行替换
    QString s = p;
    s.replace("{level}", "{0}");
...
const char* FCMessageHandlerGlobalValues_Private::getPatternChar() const
{
