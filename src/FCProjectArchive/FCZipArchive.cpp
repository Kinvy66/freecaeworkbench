...
{
//===============================================================
// FCZipArchive::PrivateData
//===============================================================
class FCZipArchive::PrivateData
{
    FC_DECLARE_PUBLIC(FCZipArchive)
public:
    PrivateData(FCZipArchive* p);
    void makeSureZipPtr();
    // 是否打开
    bool isOpened() const;
    // 获取所有文件
...
const char* FCZipArchive::PrivateData::s_password   = nullptr;
int FCZipArchive::PrivateData::s_zip_compress_level = Z_NO_COMPRESSION;

FCZipArchive::PrivateData::PrivateData(FCZipArchive* p) : q_ptr(p)
{
    makeSureZipPtr();
}

void FCZipArchive::PrivateData::makeSureZipPtr()
{
    if (!mZip) {
...
bool FCZipArchive::PrivateData::isOpened() const
{
    if (mZip && mZip->isOpen()) {
        return true;
    }
...
const char* FCZipArchive::PrivateData::password()
{
    return s_password;
}

int FCZipArchive::PrivateData::compressLevel()
{
    return s_zip_compress_level;
}

QStringList FCZipArchive::PrivateData::getAllFiles() const
{
...
bool FCZipArchive::PrivateData::ensureOpenForRead()
{
    if (mZip->isOpen() && mZip->getMode() == QuaZip::mdUnzip) {
        return true;
    }
...
bool FCZipArchive::PrivateData::ensureOpenForWrite()
{
...
QString FCZipArchive::PrivateData::errorStringFromZipError(int errorCode) const
{
    switch (errorCode) {
    case UNZ_OK:
...
bool FCZipArchive::PrivateData::copyZipEntry(QuaZip* sourceZip, QuaZip* destZip, const QString& fileName)
{
...
QString FCZipArchive::PrivateData::getSystemErrorString(int errnum)
{
#ifdef _MSC_VER
    char buffer[ 256 ] = { 0 };
...
// FCZipArchive
//===============================================================
FCZipArchive::FCZipArchive(QObject* par) : FCAbstractArchive(par), FC_PIMPL_CONSTRUCT
{
}

FCZipArchive::FCZipArchive(const QString& zipPath, QObject* par)
    : FCAbstractArchive(par), FC_PIMPL_CONSTRUCT
{
    setZipFileName(zipPath);
}

FCZipArchive::~FCZipArchive()
{
    close();
}

bool FCZipArchive::setBaseFilePath(const QString& path)
{
    FCAbstractArchive::setBaseFilePath(path);
    return setZipFileName(path);
}

bool FCZipArchive::setZipFileName(const QString& fileName)
{
    FC_D(d);
    if (isOpened()) {
        close();  // 关闭当前打开的ZIP
 ...
 * @return
 */
bool FCZipArchive::isOpened() const
{
    FC_DC(d);
    return d->mZip && d->mZip->isOpen();
}

bool FCZipArchive::close()
{
    FC_D(d);
    if (!isOpened()) {
        return true;
 ...
 * @return
 */
bool FCZipArchive::write(const QString& relatePath, const QByteArray& byte)
{
    FC_D(d);
    ...
    if (!zipFile.open(QIODevice::WriteOnly,
                      ...
                      FCZipArchive::PrivateData::s_password,
                      0,
                      Z_DEFLATED,
                      FCZipArchive::PrivateData::s_zip_compress_level)) {
 ...
 *
 * bool ok = FCZipArchive::writeFileToZip(&zip,"data/sample.txt","/home/user/sample.txt");
 *
 * if (ok) {
 *     qDebug() << "写入成功";
 * } else {
 ...
 *
 */
bool FCZipArchive::writeFileToZip(const QString& relatePath,
 ...
 * @return
 */
QByteArray FCZipArchive::read(const QString& relatePath)
{
    FC_D(d);
 ...
 */
bool FCZipArchive::readToFile(const QString& zipRelatePath,
                              const QString& localFilePath,
                              size_t chunk_mb)
{
    FC_D(d);
 ...
 * @return
 */
QStringList FCZipArchive::getAllFiles() const
{
 ...
 * @return
 */
bool FCZipArchive::contains(const QString& relatePath) const
{
 ...
 * @return
 */
bool FCZipArchive::remove(const QString& fileToRemove)
{
    FC_D(d);
    if (!isOpened() || !d->ensureOpenForRead()) {
 ...
 * @return
 */
qint64 FCZipArchive::getFileSize() const
{
 ...
 * @return
 */
int FCZipArchive::getFileCount() const
{
 ...
 * @return
 */
QString FCZipArchive::getComment() const
{
    FC_DC(d);
    if (!isOpened()) {
        qDebug() << tr("archive is not open");  // cn:文件还未打开
...
void FCZipArchive::setComment(const QString& comment)
{
    FC_D(d);
    if (!isOpened()) {
        qDebug() << tr("archive is not open");  // cn:文件还未打开
 ...
 * @return
 */
bool FCZipArchive::extractToDirectory(const QString& extractDir)
{
    FC_D(d);
    if (!isOpened() && !open()) {
...
bool FCZipArchive::compressDirectory(const QString& folderPath)
{
    FC_D(d);
    if (!isOpened() && !create()) {
        qDebug() << tr("can not open archive");  // cn:无法打开档案
...
QStringList FCZipArchive::getFileNameList() const
{
    if (!isOpened()) {
        return QStringList();
    }
 ...
 * @return
 */
QStringList FCZipArchive::getFolderFileNameList(const QString& zipFolderPath) const
{
    FC_DC(d);
    if (!isOpened()) {
        return QStringList();
...
QuaZip* FCZipArchive::quazip() const
{
 ...
 * @return
 */
QString FCZipArchive::getLastErrorString() const
{
    return d_ptr->mLastErrorString;
}

void FCZipArchive::saveAll(const QString& filePath)
{
    if (isOpened()) {
        // 如果原来已经打开需要先关闭
        close();
    ...
    if (!setBaseFilePath(tempFilePath)) {
        ...
        emit taskFinished(FCAbstractArchive::SaveFailed);
        return;
    }
    if (!create()) {
        // 创建失败，设置回来
    ...
    while (!isTaskQueueEmpty()) {
        std::shared_ptr< FCAbstractArchiveTask > task = takeTask();
        if (!task->exec(this, FCAbstractArchiveTask::WriteMode)) {
            close();
            QFile::remove(tempFilePath);
            emit taskFinished(FCAbstractArchive::SaveFailed);
            return;
        }
        emit taskProgress(task, FCAbstractArchiveTask::WriteMode);
    }
    // 创建完成关闭文件
    close();
    // 把文件替换为正式文件
    ...
    emit taskFinished(FCAbstractArchive::SaveSuccess);
}

void FCZipArchive::loadAll(const QString& filePath)
{
    if (isOpened()) {
        // 如果原来已经打开需要先关闭
        close();
    ...
    if (!setBaseFilePath(filePath) || !open()) {
        ...
        emit taskFinished(FCAbstractArchive::LoadFailed);
        return;
    }
    while (!isTaskQueueEmpty()) {
        std::shared_ptr< FCAbstractArchiveTask > task = takeTask();
        if (!task->exec(this, FCAbstractArchiveTask::ReadMode)) {
            close();
            emit taskFinished(FCAbstractArchive::LoadFailed);
            return;
        }
        emit taskProgress(task, FCAbstractArchiveTask::ReadMode);
    }
    // 加载完成后关闭
    close();
    emit taskFinished(FCAbstractArchive::LoadSuccess);
}
 ...
 * @return
 */
bool FCZipArchive::isCorrectFile(const QString& filePath)
{
    QuaZip zip(filePath);
...
bool FCZipArchive::extractToDirectory(const QString& zipFilePath, const QString& extractDir)
{
...
bool FCZipArchive::extractToDirectory(QuaZip* zip, const QString& extractDir)
{
    if (!zip->isOpen()) {
        if (!zip->open(QuaZip::mdUnzip)) {
...
bool FCZipArchive::compressDirectory(const QString& folderPath, const QString& zipFilePath)
{
    // 打开 ZIP 文件
    QuaZip zip(zipFilePath);
...
bool FCZipArchive::compressDirectory(const QString& folderPath, QuaZip* zip, const QString& relativeBase)
{
    QDir dir(folderPath);
    if (!dir.exists()) {
        return false;
 ...
 *
 */
bool FCZipArchive::writeFileToZip(QuaZip* zip, const QString& relatePath,
                                  const QString& localFilePath,
                                  std::size_t chunk_mb)
{
    // 检查本地文件是否存在
    ...
    if (!zipFile.open(QIODevice::WriteOnly, zipInfo,
                      PrivateData::s_password, 0,
                      Z_DEFLATED, PrivateData::s_zip_compress_level)) {
        localFile.close();
        return false;
    }
 ...
 * - chunk_mb 值过大会增加内存占用，过小会影响IO效率
 */
bool FCZipArchive::readToFile(QuaZip* zip, const QString& zipRelatePath,
                              const QString& localFilePath, std::size_t chunk_mb)
{
    // 参数有效性检查 ---------------------------------------------------
    if (!zip || zipRelatePath.isEmpty()
        || localFilePath.isEmpty() || chunk_mb == 0) {
        return false;
    }
