...
#include <QString>
#include <string>
#ifndef FC_STRING_CAST_D_H
#define FC_STRING_CAST_D_H(type, value) FCUTILS_API type fromQString(const QString& str, type defaultValue = value);
#endif
#ifndef FC_STRING_CAST_H
#define FC_STRING_CAST_H(type) FCUTILS_API type fromQString(const QString& str, const type& defaultValue = type());
#endif

namespace FC 
{
// 生成一个唯一的字符串，如果出现了重复字符串，将会在这个字符串后面加上_1,这个函数常用于生成一个唯一的名字
FCUTILS_API QString makeUniqueString(const QSet< QString >& stringSet,
                                     const QString& str,
                                     const QString& split = "_");
 ...
 */
FCUTILS_API std::wstring qstringToSystemWString(const QString& qstr);
FCUTILS_API std::wstring stringToSystemWString(const std::string& str);
FCUTILS_API int getSystemCodePage();
#ifdef Q_OS_WIN
FCUTILS_API std::string convertToUtf8(const std::string& str, unsigned int codePage);
#endif
// 字符串转换为值
// type fromQString(const QString& str,type defaultval = 0)
FC_STRING_CAST_D_H(short, 0)
FC_STRING_CAST_D_H(unsigned short, 0)
FC_STRING_CAST_D_H(int, 0)
FC_STRING_CAST_D_H(unsigned int, 0)
FC_STRING_CAST_D_H(long, 0)
FC_STRING_CAST_D_H(unsigned long, 0)
FC_STRING_CAST_D_H(long long, 0)
FC_STRING_CAST_D_H(unsigned long long, 0)
FC_STRING_CAST_D_H(float, 0.0f)
FC_STRING_CAST_D_H(double, 0.0)
FC_STRING_CAST_H(std::string)
FC_STRING_CAST_H(std::u16string)
FC_STRING_CAST_H(std::u32string)
FC_STRING_CAST_H(std::wstring)
